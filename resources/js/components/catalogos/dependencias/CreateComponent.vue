                    ...
                    <div class="col-md-6 offset-md-3">
                        <form autocomplete="off" class="shadow-lg p-3 mb-5 bg-white rounded" id="form" v-on:submit.prevent="submit">
                            ...
                            <div class="form-group mt-3">
                                <label for="titulo">Nombre de la dependencia</label>
                                <input class="form-control" id="titulo" name="nombre del dependencia" type="text" v-model="titulo.name" v-validate="'required'">
        ...
        methods: {
            submit() {
                this.$validator.validate().then(isValid => {
                    if(isValid) {
                        axios.post('/api/dependencias', this.titulo)
                        .then(response => {
                            Swal.fire({
                                icon: 'success',
