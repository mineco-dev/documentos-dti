    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <h1 class="text-center">Crear destinatario</h1>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            submit() {
                this.$validator.validate().then(isValid => {
                    if(isValid) {
                        axios.post('/api/destinatarios', this.destinatario)
                        .then(response => {
                            Swal.fire({
                                icon: 'success',
        ...
        mounted() {
            ...
            .then(response => {
                this.saludos = response[0].data
                this.dependencias = response[1].data
                this.entidades = response[2].data
                this.cargos = response[3].data
            })
        }
    }
</script>