<template>
    <div class="content-wrapper">
        <div class="content-header">
            <h1 class="text-dark">Modificar entidad</h1>
        </div>
        <div class="content">
                    ...
                    <div class="col-md-6 offset-md-3">
                        <form autocomplete="off" class="shadow-lg p-3 mb-5 bg-white rounded" id="form" v-on:submit.prevent="submit">
                            ...
                            <div class="form-group mt-3">
                                ...
                                <input class="form-control" id="entidad" name="nombre del entidad" type="text" v-model="entidad.name" v-validate="'required'">
...
<script>
    ...
    export default {
        ...
        mounted() {
            axios.get('/api/entidades/'+ this.$route.params.id)
            .then(response => this.entidad = response.data)
            .catch(error => {
                Swal.fire({
                    icon: 'warning',
        ...
        methods: {
            submit() {
                this.$validator.validate().then(isValid => {
                    if(isValid) {
                        axios.put('/api/entidades/' + this.entidad.id, this.entidad)
                        .then(response => {
                            Swal.fire({
                                icon: 'success',
