<template>
    <div>
        <div class="content-header">
            <h1 class="text-dark">Modificar cargo</h1>
        </div>
        <div class="content">
                ...
                <div class="row">
                    <div class="col-md-6 offset-md-3">
                        <form autocomplete="off" class="shadow-lg p-3 mb-5 bg-white rounded" id="form" v-on:submit.prevent="submit">
                            <a class="text-dark" href="#" title="Regresar a las cargos" v-on:click="$router.go(-1)">
                                <i class="fas fa-arrow-left fa-lg"></i>
                            </a>
                            <div class="form-group mt-3">
                                <label for="cargo">Nombre del cargo</label>
                                <input class="form-control" id="cargo" name="nombre del cargo" type="text" v-model="cargo.name" v-validate="'required'">
                                <div class="invalid-feedback">{{ errors.first('nombre del cargo') }}</div>
                            </div>
...
<script>
    ...
    export default {
        ...
        mounted() {
            axios.get('/api/cargos/'+ this.$route.params.id)
        ...
        methods: {
            submit() {
                this.$validator.validate().then(isValid => {
                    if(isValid) {
                        axios.put('/api/cargos/' + this.cargo.id, this.cargo)
                        .then(response => {
                            Swal.fire({
                                icon: 'success',
