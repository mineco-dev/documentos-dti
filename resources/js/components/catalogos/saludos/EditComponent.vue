<template>
    <div>
        <div class="content-header">
            <h1 class="text-dark">Modificar saludo</h1>
        </div>
        <div class="content">
...
<script>
    ...
    export default {
        ...
        mounted() {
            axios.get('/api/saludos/'+ this.$route.params.id)
            .then(response => this.saludo = response.data)
            .catch(error => {
                Swal.fire({
                    icon: 'warning',
        ...
        methods: {
            submit() {
                this.$validator.validate().then(isValid => {
                    if(isValid) {
                        axios.put('/api/saludos/' + this.saludo.id, this.saludo)
                        .then(response => {
                            Swal.fire({
                                icon: 'success',
